...
DCHISEL.resize = function() {
    ...
    if (DCHISEL.isFullScreenApp) {
        var minW = 750,
            minH = 500;
        /** Scale if too small */
        if (DCHISEL.W < minW && DCHISEL.H < minH) {
            DCHISEL.W = minW;
            DCHISEL.H = minH;
        } else if (DCHISEL.W < minW) {
            DCHISEL.W = minW;
        } else if (DCHISEL.H < minH) {
            DCHISEL.H = minH;
        }
        /** Resize if aspect ratio is bad. */
        if (DCHISEL.W < 2 * DCHISEL.H) DCHISEL.W = 2 * DCHISEL.H;
    }
