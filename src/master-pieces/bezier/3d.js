 ...
 */
  ...
  RIPSAW.Bezier3D.prototype = {
    ...
    toSVG: function () {
      ...
      result += ' ' + points[0].toSVG() + '' + points[1].toSVG()

      result += '<g>'

      for (i = 0; i < max; i += 5) {
        result += new RIPSAW.Line(points[0].getPoint(i), points[1].getPoint(i)).toSVG()
      }

      result += '</g>'

      return RIPSAW.textAssets.SVGHeader + result + RIPSAW.textAssets.SVGFooter
    },
